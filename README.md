...
### 3.1 연구 배경
초미세먼지(PM2.5)는 우리의 호흡기 건강 및 일상생활의 질을 결정짓는 핵심적인 환경 지표이다. 미세먼지 농도를 단기적으로 세밀하게 예측할 수 있다면 취약계층 보호 및 신속한 공공 경보 발령 등 여러 방면에서 큰 도움이 될 것이다. 일반적으로 초미세먼지는 강수량, 풍속 등 복잡한 기상 조건의 영향을 크게 받는다. 하지만 본 프로젝트에서는 주변 대기 측정소에서 매일 관측되는 가스 성분(NO2, O3, CO, SO2 등) 농도 데이터만으로도 의미 있는 수준의 초미세먼지 예측 모델을 구축할 수 있을지 그 가능성을 탐구해 보고자 하였다.

### 3.2 프로젝트 목표
- **초미세먼지 단기 예측 모델 구현**: 흐르는 시간(월, 시간대) 데이터와 이산화질소, 일산화탄소 등 측정된 기초 가스 데이터만을 온전히 활용해 PM2.5 농도를 산출해 내는 머신러닝 시스템 개발
- **알고리즘 성능 비교 및 장단점 분석**: 가중치를 규제해 서로 얽힌 가스 변수 간의 다중공선성 문제를 완화하는 선형 모델(Ridge, Lasso)과, 복잡한 비선형적 관계를 꿰뚫어 보는 앙상블 모델(Random Forest)의 성능을 객관적으로 비교 평가
- **핵심 유발 요인 및 시계열 특성 파악**: 1년 중 어느 계절에, 하루 중 어느 시간대에 초미세먼지가 극심해지는지를 파악하기 위한 파생 변수(Feature)를 설계하고 예측에 제일 큰 영향을 준 가중 요인 시각화
...
두 모델 모두 가중치 크기를 억제하여 과적합을 방지한다는 목적은 동일하지만, 비용 함수에 적용하는 **페널티의 형태(L2와 L1)**에 따라 가중치가 규제되는 수학적 과정과 결과물에서 결정적인 차이가 발생합니다.

- **기하학적 제약 공간과 가중치 축소(Shrinkage) 방식의 차이**:
  - **릿지(Ridge - L2 Norm)**: 독립 변수의 가중치 제곱합을 벌점으로 사용하기 때문에, 최적화 과정에서 제약 공간이 수학적으로 **원(원점 중심의 구)** 형태를 띱니다. 이 둥근 곡면과 오차 함수의 타원이 만나는 교점에서 최적해가 정해지므로, 변수의 가중치가 0에 한없이 가까워지기는 하지만 **완전히 0이 되지는 않습니다**. 따라서 **모든 가스 및 시간 변수를 모델에 유지**한 채 전반적인 가중치 덩치만 줄여 예측을 수행하며, 서로 유기적으로 엮여있는 가스 농도들 사이의 다중공선성을 부드럽게 완화하여 전체적인 모델 학습을 안정화하는 데 탁월합니다.
  - **라쏘(Lasso - L1 Norm)**: 가중치의 절댓값 합을 벌점으로 사용하므로, 제약 공간이 모서리가 뾰족한 **마름모(다이아몬드)** 형태를 띱니다. 오차 등고선이 이 마름모와 만날 때 수학학적으로 뾰족한 꼭짓점(특정 가중치가 정확히 0인 축)에서 교차할 확률이 매우 높습니다. 그 결과 예측 영향력이 적은 변수의 가중치를 **완전히 '0'으로 탈락**시켜버립니다. 이를 통해 초미세먼지 농도에 직접적으로 영향을 주는 소수의 핵심 변수만 남기는 **자동 변수 선택(Feature Selection)** 효과를 불러와 모델의 직관적 해석을 용이하게 합니다. 다만 상관관계가 높은 다수의 가스 변수가 있을 때 그중 하나만 임의로 살리고 나머지를 날려버릴 위험성도 함께 내포하고 있습니다.
...
- **Random Forest 회귀 모델 구축**:
    비선형적인 시계열 데이터 패턴과 가스 모델 간 상호작용을 효과적으로 학습하기 위해 `RandomForestRegressor` 모델을 설계하였다. 무작위 트리의 개수는 100개(`n_estimators=100`)로 설정하여 앙상블 결과의 안정성을 도모하였고, 각 트리가 훈련 데이터의 노이즈까지 깊게 학습하는 과적합(Overfitting)을 방지하기 위해 트리의 뻗어나가는 깊이를 10(`max_depth=10`)으로 엄격히 제한하였다. 또한 노드를 분할할 때 사용할 변수의 수를 결정하는 `max_features` 파라미터는 `scikit-learn`의 기본값을 그대로 적용함으로써, 모델이 매 분할마다 변수를 축소하지 않고 모든 가스 및 시간 데이터를 온전히 고려하여 최대한 정확한 형태의 패턴을 학습하도록 설계하였다.
